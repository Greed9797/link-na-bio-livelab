...
import type { FormDef, FormValues } from "@/lib/forms/types";
import { buildWhatsappUrl, CONTACTS } from "@/lib/whatsapp";
...
export default function FormShell({ form }: { form: FormDef }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/submit/${form.persona}`, {
        method: "POST",
    ...
    return (
      <Shell>
        <div className="flex flex-col items-center gap-[20px] py-[40px] text-center">
          ...
          <Link href="/" className="mt-[8px] text-[13px] text-white/60 underline-offset-4 hover:underline">
            Voltar para a home
          </Link>
        </div>
      </Shell>
    ...
    return (
      <Shell>
        <div className="flex flex-col items-center gap-[20px] py-[40px] text-center">
          ...
          <Link href="/" className="text-[12px] text-white/50 underline-offset-4 hover:underline">
            Voltar
          </Link>
        </div>
      </Shell>
  ...
  return (
    <main className="relative min-h-screen bg-black">
      <div className="absolute inset-0 md:hidden">
        <Image src="/bg-mobile.jpg" alt="" fill priority className="object-cover object-center" sizes="100vw" />
      </div>
      <div className="absolute inset-0 hidden md:block">
        <Image src="/bg-desktop.jpg" alt="" fill priority className="object-cover object-center" sizes="100vw" />
      </div>
